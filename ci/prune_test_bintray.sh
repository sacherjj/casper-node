...
get_help() {
  echo -e "Usage: $0 --repo-name <NAME> --package-name <PACKAGE_NAME> [--package-version <VERSION>] [--prune-number <NUMBER>]\n"
  echo -e "Example: $0 --repo-name debian --package-name [casper-node|casper-client]\n"
  echo "Note: If --package-version is not set current node version will be used."
}
